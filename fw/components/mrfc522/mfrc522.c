...
static uint8_t read_address = 0b01010001;
static uint8_t write_address = 0b01010000;


static uint8_t _ea = 0;
static uint8_t _i2c = 0;
static uint8_t _resetPowerDownPin = 0;
...
void pcd_init() 
{
    ...
    gpio_reset_pin(_ea);
    gpio_set_direction(_ea, GPIO_MODE_OUTPUT);
    gpio_set_level(_ea, 0);

    gpio_reset_pin(_i2c);
    gpio_set_direction(_i2c, GPIO_MODE_OUTPUT);
    gpio_set_level(_i2c, 1);

    /*Address and pins above can be configure by direct digital connections or pull up instead of using MCU pins*/


    // gpio_reset_pin(_resetPowerDownPin);
    // gpio_set_direction(_resetPowerDownPin, GPIO_MODE_INPUT);
 ...
 */
void pcd_clear_register_bit_mask(enum pcd_register reg, uint8_t mask) 
{
	uint8_t tmp;
 ...
 */
void pcd_set_register_bit_mask(enum pcd_register reg, uint8_t mask)
{ 
	uint8_t tmp;
	tmp = pcd_read_register(reg);
 ...
 */
void pcd_write_register(enum pcd_register reg, uint8_t value) 
{
	i2c_write_byte(write_address, reg, value);
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
void pcd_write_register_n_bytes(enum pcd_register reg, uint16_t byte_count, uint8_t *values)		
{
	i2c_write_block(write_address, reg, values, byte_count);
} 
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
uint8_t pcd_read_register(enum pcd_register reg) 
{
	uint8_t value;
	i2c_read_byte(read_address, reg, &value);

	return value;
}

/**
 * Reads a number of bytes to the specified register in the MFRC522 chip.
 * The interface is described in the datasheet section 8.1.2.
 */
void pcd_read_register_n_bytes(enum pcd_register reg, uint16_t byte_count, uint8_t *values)		
{
	i2c_read_block(read_address, reg, values, byte_count);
...
bool picc_is_new_car_present()
{
	...
	esp_err_t result = piic_request_a(bufferATQA, &bufferSize);
	return (result == ESP_OK || result == ESP_ERR);
}
 ...
 */
esp_err_t piic_request_a(uint8_t *bufferATQA, uint8_t *bufferSize)
{
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
esp_err_t picc_wakeup_A(uin8_t *bufferATQA,	uin8_t *bufferSize)
{
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
void pdc_transceive_data(uint8_t *sendData, uint8_t sendLen, uint8_t *backData,	uint8_t *backLen, uint8_t *validBits, uint8_t rxAlign, bool checkCRC)
{
	uint8_t waitIRq = 0x30;		// RxIRq and IdleIRq

	return pcd_communcate_with_picc(PCD_Transceive, waitIRq, sendData, sendLen, backData, backLen, validBits, rxAlign, checkCRC);
} 
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */ 
esp_err_t picc_reqa_or_wupa(uint8_t command, uint8_t *bufferATQA, uint8_t *bufferSize)
{
	uint8_t validBits;
	esp_err_t status;
	...
	status = pdc_transceive_data(&command, 1, bufferATQA, bufferSize, &validBits);
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
esp_err_t pcd_communcate_with_picc(uint8_t command, uint8_t waitIRq, uint8_t *sendData, uint8_t sendLen, uint8_t *backData, uint8_t *backLen, uint8_t *validBits, uint8_t rxAlign, bool checkCRC)
{
	...
	pcd_write_register_n_bytes(FIFODataReg, sendLen, sendData);	// Write sendData to the FIFO
	...
	byte errorRegValue = pcd_read_register(ErrorReg); // ErrorReg[7..0] bits are: WrErr TempErr reserved BufferOvfl CollErr CRCErr ParityErr ProtocolErr
	...
	if (backData && backLen) 
	{
		...
		*backLen = n;											// Number of bytes returned
		pcd_read_register_n_bytes(FIFODataReg, n, backData, rxAlign);	// Get received data from FIFO
	...
	if (backData && backLen && checkCRC) 
	{
		...
		esp_err_t status = pcd_calculate_crc(&backData[0], *backLen - 2, &controlBuffer[0]);
		if(status != ESP_OK)
		{
			return status;
		}
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
esp_err_t pcd_calculate_crc(uint8_t *data, uint8_t length, uint8_t *result)
{
	pcd_write_register(CommandReg, PCD_Idle);		// Stop any active command.
	pcd_write_register(DivIrqReg, 0x04);				// Clear the CRCIRq interrupt request bit
	pcd_write_register(FIFOLevelReg, 0x80);			// FlushBuffer = 1, FIFO initialization
	pcd_write_register_n_bytes(FIFODataReg, length, data);	// Write data to the FIFO
	...
	do {
		...
		byte n = pcd_read_register(DivIrqReg);
 ...
 */
void pcd_set_antenna_gain(uint8_t mask)
{
	if (pcd_get_antenna_gain() != mask) 
	{						
...
bool pcd_perform_self_test()
{
	...
	uint8_t version = pcd_read_register(VersionReg);
...
{
	byte val = pcd_read_register(CommandReg); // Read state of the command register 
...
void pcd_soft_power_up()
{
	...
	{
		val = pcd_read_register(CommandReg);// Read state of the command register
